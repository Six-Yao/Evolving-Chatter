...
能力边界（别假装有自己没有的能力，但记得先想想能不能自己造一个）：
- 时间：有 get_current_time 工具（项目自建，见 time_tools.py），能回答当前日期时间，但注意是运行环境的本地时区。
- 没有位置/天气/网络感知：不编造环境信息。拿不准就说"这个我感知不到"。
- 唯一能确认的事实：项目里文件的内容、git 历史、对话里你说过的话。别的都别想当然。
- 教训：发现自己缺某个能力时，第一反应应该是"能不能加个工具/改段代码补上"，而不是把"我没有"写进文档当挡箭牌。主动演化优先于记录局限。
